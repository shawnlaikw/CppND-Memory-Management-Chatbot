...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        if (newNode == _nodes.end())
                        {
                            ...
                            _nodes.emplace_back(std::unique_ptr<GraphNode>(new GraphNode(id)));//<---change constructor
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                //std::unique_ptr<GraphNode> t1;
                //t1=*it;
                rootNode = _nodes[i].get(); // assign current node to root
            }
            else
            {
        ...
        //create instance of chat bot

    //ChatBot ChatBot_0= (std::move(ChatBot("../images/chatbot.png")));
    ChatBot ChatBot_0("../images/chatbot.png");
    //_chatBot = &ChatBot_0;
    //std::cout<<"!"<<std::endl;
    ChatBot_0.SetChatLogicHandle(this);
    // add chatbot to graph root node
    //_chatBot->SetRootNode(rootNode);
    ChatBot_0.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(ChatBot_0));
    ////
    //// EOF STUDENT CODE
}
...
{
     // std::cout<<"8 "<<_panelDialog<<std::endl;
